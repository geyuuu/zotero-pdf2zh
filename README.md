...
[![zotero target version](https://img.shields.io/badge/Zotero-7--10-green?style=flat-square&logo=zotero&logoColor=CC2936)](https://www.zotero.org/download/)
...
2. 插件设置 →「本地翻译服务」：勾选「翻译前自动启动本地服务」，`server 文件夹路径` **留空**（用内置），启动方式保持 `uv`（uv 路径留空会自动查找，如果找不到需要自己设置）。
3. 右键 PDF → PDF2zh → 翻译。首次会自动启动后端并下载依赖（需几分钟），之后即用即翻。
4. 解析PDF和翻译需要时间。在工具栏点击图标可以打开后端网页查看进度。
